...
Only **two** things are fixed: the **model** (the proxy, the only LLM you may call) and the **world** (AppWorld via the gateway tools and the oracle). Everything else is yours to build and **bundle in this repo**: your own memory (gbrain, a vector DB, sqlite, a skill library), your own RAG, your own MCP servers and tools, any framework. It all runs locally in your container.

The sandbox has **no open internet** (anti-cheat: nobody routes reasoning to a bigger model), so **bundle your stack to run offline** — that does *not* mean "no gbrain or MCPs." gbrain is Postgres plus an MCP server; it runs locally fine. Show what you build with a real engineering stack around a deliberately weak model.

## What you must use

This is non-negotiable, and it's what we grade for:

1. **MCP tools** — act through a tool surface, not hardcoded calls. Five tools: `search_apis`, `api_doc`, `call_api`, `run_code`, `complete_task`. (Bring your own MCP servers on top if you want.)
...
5. **Memory is yours, and required** — a store that **compounds across tasks**, plus self-correction (verify, reflect, retry). `FLYWHEEL_MEMORY_DIR` is a persistent dir that survives the whole task stream; we never wipe it. Put your own store there (the starter SDK ships a JSON store; swap it for a vector index, sqlite, or gbrain bundled in your image). A real memory reuses prior API docs, login recipes, and fixes, and that reuse raises your score. You defend it in your writeup and in the call. A decorative `memory.json` nobody reads gives no lift.
...
def solve(ctx):
    ...
    # ctx.memory.read()/.write(k, v)    a starter store under FLYWHEEL_MEMORY_DIR; bundle your own
...
| `FLYWHEEL_MEMORY_DIR` | a persistent dir that survives across the task stream; **your** store lives here |
...
those gateways**, not from files you write, so act through `ctx` (not hardcoded HTTP). Memory is
yours: it is a persistent dir, not a gateway, so what crosses tasks is whatever you write to it.
...
Every practice and grade run returns, **per task**: your agent's stdout/stderr, the gateway trace (the `call_api`/`run_code` calls and the errors they threw), and the oracle's pass/fail with the exact **reason it failed** (the failing check / missing state mutation), plus timing and tokens. Practice is **unlimited** and exists for this loop: read why each task failed, fix that, rerun. The API response and the SPA both surface these per-task logs.
